...
//调用
    renderData(topicLavoro);

})
// .catch(err => console.log('Error loading or parsing data.'));


function renderData(topicLavoro){
    radialTree(topicLavoro, "上海文化物质遗产");
}
...
{ 
    ...
    const svg = d3.select("body").append("svg")//.attr(svg(width, width))
    .style("width", "40%")
    .style("height", "auto")
    .style("padding", "10px")
    ...
    d3.select("body").appendChild(svg.node());
